...
class Instance(object):
    ...
    def attach_nic(self, conf, vif):
        ...
        dpm_nic_dict = {
            ...
            "virtual-switch-uri": "/api/virtual-switches/"
                                  + dpm_object_id,
            "device-number": "1800"
            # TODO(preethipy) only for dp1 demo the value is
            # hardcoded to 1800 but this can be any number
        }
    ...
    def attachHba(self, conf):
        ...
        for adapterPort in mapping.get_adapter_port_mapping():
            ...
            device_number = adapterPort['device-number']
            dpm_hba_dict = {
    ...
    def get_hba_wwpns(self):
        LOG.debug('Get Hba wwpns')
        hba_wwpns = []
        if self.partition is not None:
            hba_manager = zhmcclient.HbaManager(self.partition)
            hbas = hba_manager.list(full_properties=False)
            for hba in hbas:
                hba_wwpns.append(hba.properties['wwpn'])
        return hba_wwpns
    ...
    def destroy(self):
        ...
        if self.partition:
            ...
            # TODO(preethipy): The below method to be removed once the bug
            # on DPM(701894) is fixed to return correct status on API
            # return
