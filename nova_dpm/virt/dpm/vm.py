...
class PartitionInstance(object):
    ...
    def _append_nic_to_boot_os_specific_parameters(self, nic, vif):
        ...
        * Configure the physical network adapter port to be used.
        ...
        # We only set this parameter if it is valid. And do not delete the
        # already created NIC. So it's up to the caller to decide whether
        # to proceed without having the NIC configured in the boot properties.
    ...
    def attach_nic(self, conf, vif):
        ...
        # while adding it to the boot parameters, as deleting the partition
