...
class VmCreateTestCase(TestCase):
    ...
    def test_set_boot_properties(self):
        partition_inst = vm.PartitionInstance(
            self.instance, self.cpc, self.flavor)
        partition_inst.create(partition_inst.properties())
        partition = partition_inst.get_partition()
        wwpn = '500507680B214AC1'
        lun = 1
        booturi = partition.get_property('object-uri') + '/hbas/1'
        partition_inst.set_boot_properties(wwpn, lun, booturi)
    ...
    def test_get_boot_properties(self):
        ...
        self.assertEqual(
            ...
            partition_inst.get_boot_properties()['boot-device'])
