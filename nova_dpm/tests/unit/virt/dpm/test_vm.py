...
class VmFunctionTestCase(TestCase):
    ...
    def test_append_to_boot_os_specific_parameters(self):
        mock_part = mock.Mock()
        ...
        inst.append_to_boot_os_specific_parameters("bar")
        mock_part.update_property.assert_called_with(
            **{"boot-os-specific-parameters": "foobar"})

    def test_append_to_boot_os_specific_parameters_exceeded(self):
        mock_part = mock.Mock()
        mock_part.get_property.return_value = "a" * 256
        with mock.patch.object(vm.PartitionInstance, 'get_partition',
                               return_value=mock_part):
            inst = vm.PartitionInstance(None, None, None)
