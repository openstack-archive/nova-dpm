...
class VmCreateTestCase(TestCase):
    ...
    def test_attach_hbas(self):
        ...
        adapter_object_id = adapter.get_property('object-id')
        port_element_id = adapter.ports.list()[0].get_property('element-id')
        storage = adapter_object_id + ":" + port_element_id
        self.flags(group="dpm", physical_storage_adapter_mappings=[storage])
        partition_inst = vm.PartitionInstance(
            self.instance, self.cpc, self.flavor)
        partition_inst.create(partition_inst.properties())
        partition_inst.attach_hbas()
        self.assertEqual(1, len(partition_inst.get_partition().hbas.list()))
