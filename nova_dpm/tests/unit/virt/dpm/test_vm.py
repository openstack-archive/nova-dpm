...
class VmPartitionInstanceTestCase(TestCase):
    ...
    def test_set_boot_os_specific_parameters_negative(self):
        data = '1800,0,' \
               'fa163ee49a98a163ee49a98a' \
               '163ee49a98a163ee49a98a163ee4' \
               '9a98a163ee49a98a163ee49a98a163ee' \
               '49a98a163ee49a98a163ee49a98a163' \
               'ee49a98a163ee49a98a163ee49a98' \
               'a163ee49a98a163ee49a98a163ee49a98a163e' \
               'a163ee49a98a163ee49a98a163ee49a98a163ee49a98' \
               'a163ee49a98a163ee49a98a163ee49a98a163ee49a98' \
               'a163ee49a98a163ee49a98a163ee49a98a163ee49a98' \
               'a163ee49a98a163ee49a98a163ee49a98a163ee49a98e49a98;'
    ...
    def test_get_boot_properties(self):
        self.assertEqual(
            'test-operating-system',
            self.partition_inst.get_boot_properties()['boot-device'])
...
class PhysicalAdapterModelTestCase(TestCase):
    ...
    def test_parse_config_line(self):
        adapter = "48602646-b18d-11e6-9c12-42f2e9ef1641"
        port_num = "0"
        line = adapter + ":" + port_num
        adapter_id, port = self.phycal_adapter.parse_config_line(line)
        self.assertEqual(adapter, adapter_id)
        self.assertEqual(port_num, str(port))
...
class PartitionInstanceInfoTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.create_partition_properties = {
            'name': self.part_name,
            'description': self.part_description,'initial-memory': 1,'status':self.state ,'maximum-memory': 512,'ifl-processors':2}

        # Create partition in a cpc not from openstack
        # and used same uuid of instance to create
        # vm.PartitionInstanceInfo class instance to get the information of the status,memory etc..,

        partition = self.cpc.partitions.create(
            self.create_partition_properties)


        self.instance_partition=vm.PartitionInstanceInfo(self.instance,self.cpc)
    ...
    def test_cpu_time(self):
        ...
        #Hardcoded-value for the cpu-time in vm.py
         pass
