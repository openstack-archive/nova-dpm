...
class DPMdriverInitHostTestCase(TestCase):
    ...
    @mock.patch.object(client_proxy, 'get_client_for_sesion')
    def test_host(self, mock_client_proxy, mock_warning):
        self.flags(group="dpm", hmc="1.1.1.1")
        self.flags(group="dpm", hmc_username="dummyuser")
        dummyvirtapi = None
        dpmdriver = driver.DPMDriver(dummyvirtapi)
        mock_client_proxy.assert_called_with("1.1.1.1","dummyuser",None)
